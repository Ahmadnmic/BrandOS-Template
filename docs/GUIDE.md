...
API key (`FIRECRAWL_API_KEY=fc-…`). The key stays in `.env`, which is
gitignored — never share it in chat, never commit it. Static/SSR client
sites work without a key (direct mode).
...
The CVI is law for rules; the site is evidence of what exists. Where they
disagree (the site uses a blue the CVI doesn't contain; the CVI mandates a
font the site never loads), the agent writes each conflict to
`intake/reconciliation.md` and asks you to decide. It never silently picks a
side, and it never invents values found in neither input.
...
- **Updating the brand:** edits go through `brand/` + `content/`, bump the
  version in `brand.config.ts`, add a changelog entry. Subscribers get a
  digest; developers get a new `@client/brand-tokens` version + token diff.
- **Access:** manage the partner allowlist (external agencies, with expiry
  dates) in `brand.config.ts` — offboarding is deleting a line.
