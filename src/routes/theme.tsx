...
export default function ThemeQA() {
  return (
    <div className="mx-auto max-w-4xl">
      <ChapterHead num="QA" title="Theme" steps="ALLE TOKENS · ALLE TILSTANDE" />
      <p className="mb-6 max-w-xl text-sm text-dim">
        QA-fladen efter hver generate-theme-kørsel: alle sys-roller malet live fra CSS-variablerne.
        Skift Lys/Mørk i headeren og se hele fladen følge med.
      </p>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        {SWATCH_VARS.map((name) => (
          <div key={name} className="overflow-hidden rounded-md border border-line">
            <div className="h-16" style={{ background: `var(--sys-${name})` }} />
            <div className="bg-panel px-2.5 py-2 font-mono text-[9.5px] text-dim">--sys-{name}</div>
          </div>
        ))}
      </div>
      <div className="mt-8 flex flex-wrap items-center gap-3 rounded-md border border-line bg-panel p-6">
