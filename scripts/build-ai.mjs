...
// Extract the guide's usage rules into machine-readable data. The sections
// are the single source; this artifact is regenerated every build and feeds
// the brand MCP's get_rules/check tools and any future compliance pass.
const rules = [];
const ruleSources = [
...
const TX_PAIR =
  /tx\(\{\s*da:\s*"((?:[^"\\]|\\.)*)"\s*,\s*en:\s*"((?:[^"\\]|\\.)*)"\s*,?\s*\}\)/g;
for (const dir of ruleSources)
for (const f of fs.existsSync(dir) ? fs.readdirSync(dir) : []) {
  if (!f.endsWith(".tsx")) continue;
  const src = fs.readFileSync(path.join(dir, f), "utf8");
  for (const block of src.matchAll(/(dos|donts)=\{\[\s*([\s\S]*?)\]\}/g)) {
    const kind = block[1] === "dos" ? "do" : "dont";
    for (const m of block[2].matchAll(TX_PAIR)) {
      rules.push({
        chapter: f.replace(".tsx", ""),
        kind,
        da: m[1].replace(/\\"/g, '"'),
        en: m[2].replace(/\\"/g, '"'),
      });
...
const rulesArtifact = {
  $description:
    "Generated from src/sections by build-ai: every GØR/UNDGÅ rule as data. Regenerated each build; never hand-edit.",
  count: rules.length,
  rules,
};
fs.writeFileSync(
  path.join(root, "brand", "rules.json"),
...
// Emit the per-brand agent skill so any agent working in this repo (or a
// clone) loads the brand's law automatically.
const skillDir = path.join(root, ".claude", "skills", "brand");
fs.mkdirSync(skillDir, { recursive: true });
fs.writeFileSync(
  path.join(skillDir, "SKILL.md"),
  `---
name: brand
description: Answer questions about the ${brandName} brand and produce on-brand material. Use whenever a task involves ${brandName}'s identity, colors, typography, voice, components or rules. Generated by build-ai from the brand's own data; regenerated every build.
---

# ${brandName}, the brand as law
...
Rules in force: ${rules.length} usage rules across ${new Set(rules.map((r) => r.chapter)).size} chapters. Writing: no em or en dashes as pause marks, no AI-tell phrases; the brand's languages are in brand.config.ts langs.
`,
);
