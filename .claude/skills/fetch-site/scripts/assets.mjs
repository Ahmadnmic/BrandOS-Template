...
function normalizeAssetUrl(raw) {
  try {
    ...
    if (
      ...
      [...u.searchParams.keys()].some((k) => RESIZE_PARAMS.has(k.toLowerCase()))
    ) {
      u.search = "";
    }
    return u.href;
...
function localNameFor(u, category) {
  ...
  const segs = url.pathname
    .split("/")
    .filter(Boolean)
    .map((s) => decodeURIComponent(s).replace(/[^a-z0-9._-]/gi, "_"));
  let name = segs.pop() || "index";
  const prefix = segs.length ? segs[segs.length - 1] : "";
  if (
    ["images", "videos", "other"].includes(category) &&
    prefix &&
    !/^(media|images?|img|static|assets|_next|_nuxt|uploads|files)$/i.test(
      prefix,
    )
  ) {
    name = `${prefix}--${name}`;
  ...
  const dir =
    category === "css"
      ? path.join(cfg.out, "css")
      : category === "js"
        ? path.join(cfg.out, "js")
...
function register(rawUrl, base, forcedCategory = null) {
  ...
  if (u.startsWith("data:") || u.startsWith("blob:") || u.startsWith("#"))
    return;
  try {
    u = new URL(u, base || cfg.origin).href;
  } catch {
    return;
  }
  ...
  assetMap.set(norm, {
    full,
    category: cat,
    rel: path.relative(cfg.out, full).replace(/\\/g, "/"),
  });
}
...
const manifest = fs.existsSync(cfg.meta("scrape-manifest.json"))
  ? readJSON(cfg.meta("scrape-manifest.json"))
  : {};
...
for (const f of pageFiles) {
  ...
  $('link[rel="stylesheet"]').each((_, el) =>
    register($(el).attr("href"), base, "css"),
  );
  $(
  ...
  ).each((_, el) => register($(el).attr("href"), base, "images"));
  $('link[rel="manifest"]').each((_, el) =>
    register($(el).attr("href"), base, "other"),
  );
  ...
  $("img").each((_, el) => {
    for (const attr of [
      ...
      register($(el).attr(attr), base);
    for (const attr of ["srcset", "data-srcset", "data-lazy-srcset"])
      harvestSrcset($(el).attr(attr), base);
  });
  $("source").each((_, el) => {
    register($(el).attr("src"), base);
    harvestSrcset($(el).attr("srcset"), base);
    harvestSrcset($(el).attr("data-srcset"), base);
  });
  // lazy background patterns
  $("[data-bg], [data-background], [data-background-image]").each((_, el) => {
    for (const attr of ["data-bg", "data-background", "data-background-image"])
      register($(el).attr(attr), base);
  });
  // noscript fallbacks often hold the real <img> for lazy loaders
  $("noscript").each((_, el) => {
    const inner = cheerio.load($(el).text() || $(el).html() || "");
    inner("img").each((_, im) => {
      register(inner(im).attr("src"), base);
      harvestSrcset(inner(im).attr("srcset"), base);
    });
  });
  // SVG sprite references: <use href="/sprite.svg#logo"> — fetch the sprite file
  $("use").each((_, el) => {
    const href = $(el).attr("href") || $(el).attr("xlink:href") || "";
    const file = href.split("#")[0];
    if (file) register(file, base, "images");
  });
  $("video").each((_, el) => {
    register($(el).attr("src"), base, "videos");
    register($(el).attr("poster"), base, "images");
  });
  $("audio").each((_, el) => register($(el).attr("src"), base));
  $('meta[property="og:image"], meta[name="twitter:image"]').each((_, el) =>
    register($(el).attr("content"), base, "images"),
  );
  $("a[href]").each((_, el) => {
    const h = ($(el).attr("href") || "").split("?")[0];
    if (DOC_EXT.test(h)) register($(el).attr("href"), base, "other");
  });
  $("[style]").each((_, el) => {
    for (const m of ($(el).attr("style") || "").matchAll(
      /url\((['"]?)([^'")]+)\1\)/g,
    ))
      register(m[2], base);
  });
...
const localToRemote = new Map(
  [...assetMap.entries()].map(([u, m]) => [m.full, u]),
);
...
await run(
  [...assetMap.entries()].slice(before2),
  `round2 (css refs, ${assetMap.size - before2} new)`,
);
...
if (payloadRefs.length)
  console.log("[assets] payloads saved:", payloadRefs.length);
...
for (const [dir] of payloadRefs) {
  ...
  const txt = fs
    ...
    .replace(/\\u002F/gi, "/")
    .replace(/\\\//g, "/");
  for (const m of txt.matchAll(/https?:\/\/[a-z0-9.-]+\/[^"\\\s)]+/gi))
    register(m[0]);
}
await run(
  [...assetMap.entries()].slice(before3),
  `round3 (payload media, ${assetMap.size - before3} new)`,
);
...
for (const f of [...pageFiles, ...walk(cfg.pages, "payload.json")]) {
  ...
  const route =
    "/" +
    path
      .relative(cfg.pages, path.dirname(f))
      .replace(/\\/g, "/")
      .replace(/^home$/, "");
  for (const m of txt.matchAll(/player\.vimeo\.com\/video\/(\d+)/g))
    (
      vids.get("vimeo:" + m[1]) ||
      vids.set("vimeo:" + m[1], new Set()).get("vimeo:" + m[1])
    ).add(route);
  for (const m of txt.matchAll(
    /(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/g,
  ))
    (
      vids.get("youtube:" + m[1]) ||
      vids.set("youtube:" + m[1], new Set()).get("youtube:" + m[1])
    ).add(route);
}
if (vids.size) {
  ...
  fs.writeFileSync(
    path.join(cfg.out, "assets", "videos", "embeds.md"),
...
// Usability summary: images/logos are build inputs, so a run must know
// whether it actually has any, and which pages yielded none.
const byCat = {};
for (const meta of assetMap.values())
  byCat[meta.category] = (byCat[meta.category] || 0) + 1;
const zeroImagePages = [];
for (const f of pageFiles) {
  const html = fs.readFileSync(f, "utf8");
  if (!/<img[\s>]/i.test(html) && !/background-image/i.test(html)) {
    zeroImagePages.push(
      "/" + path.relative(cfg.pages, path.dirname(f)).replace(/\\/g, "/"),
    );
  }
}
writeJSON(cfg.meta("asset-summary.json"), {
  countsByCategory: byCat,
  imagesTotal: byCat.images || 0,
  zeroImagePages: zeroImagePages.slice(0, 40),
  zeroImagePageCount: zeroImagePages.length,
  skippedExternalHosts: Object.fromEntries(
    [...externalHosts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 15),
  ),
});
if (!byCat.images)
  console.log(
    "[assets] WARNING: ZERO images captured. Review skipped external hosts and escalate per the IMAGES & LOGOS GUARANTEE",
  );
if (zeroImagePages.length > pageFiles.length / 2)
  console.log(
    `[assets] WARNING: ${zeroImagePages.length}/${pageFiles.length} pages carry no <img>, likely JS-lazy loading; escalate those pages to rendered capture`,
  );
writeJSON(
  cfg.meta("external-hosts.json"),
  Object.fromEntries([...externalHosts.entries()].sort((a, b) => b[1] - a[1])),
);
console.log("[assets] DONE total:", assetMap.size);
const ext = [...externalHosts.entries()]
  .sort((a, b) => b[1] - a[1])
  .slice(0, 10);
if (ext.length)
  console.log(
    "[assets] skipped external hosts (rerun with --asset-hosts if one is the site's CDN):\n" +
